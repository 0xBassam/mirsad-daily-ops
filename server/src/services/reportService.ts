...
export async function generateFloorCheckPDF(floorCheckId: string, res: Response, organizationId?: string, orgName?: string): Promise<void> {
  ...
  // Header
  doc.fontSize(20).font('Helvetica-Bold').text((orgName || 'Mirsad').toUpperCase(), { align: 'center' });
  ...
  doc.moveDown(2);
  doc.fontSize(8).fillColor('grey').text(`Generated by ${orgName || 'Mirsad'} — ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, { align: 'center' });
  doc.end();
}
