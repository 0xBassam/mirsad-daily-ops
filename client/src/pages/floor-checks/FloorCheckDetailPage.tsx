  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-slate-900">Floor Check — {formatDate(check.date)}</h1>
        <StatusBadge status={check.status} />
      </div>

      {/* Header info */}
      <div className="card p-5 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div><span className="text-slate-500">Project</span><p className="font-medium">{typeof check.project === 'object' ? check.project.name : '-'}</p></div>
        <div><span className="text-slate-500">Building</span><p className="font-medium">{typeof check.building === 'object' ? check.building.name : '-'}</p></div>
        <div><span className="text-slate-500">Floor</span><p className="font-medium">{typeof check.floor === 'object' ? check.floor.name : '-'}</p></div>
        <div><span className="text-slate-500">Supervisor</span><p className="font-medium">{typeof check.supervisor === 'object' ? check.supervisor.fullName : '-'}</p></div>
        <div><span className="text-slate-500">Shift</span><p className="font-medium capitalize">{check.shift}</p></div>
        <div><span className="text-slate-500">Next Step</span><p className="font-medium capitalize">{check.currentApprovalStep?.replace(/_/g, ' ')}</p></div>
        {check.notes && <div className="col-span-2"><span className="text-slate-500">Notes</span><p>{check.notes}</p></div>}
      </div>

      {/* Lines table */}
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200">
          <h2 className="font-semibold text-slate-800">Check Lines</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>{['Item', 'Unit', 'Planned', 'Actual', 'Diff', 'Status', 'Notes'].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">{h}</th>)}</tr>
            </thead>
      ...
      {(check.approvalRecords as any[])?.length > 0 && (
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200">
            <h2 className="font-semibold text-slate-800">Approval History</h2>
          </div>
      ...
      {check.status === 'approved' && (
        <div className="card p-5">
          <h2 className="font-semibold text-slate-800 mb-3">Export Report</h2>
