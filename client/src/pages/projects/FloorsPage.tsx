...
export function FloorsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['floors', page], queryFn: () => apiClient.get('/floors', { params: { page, limit: 50 } }).then(r => r.data) });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['floors'] }); toast.success(t('common.save')); setShowModal(false); },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">{t('nav.floors')}</h1>
        <button onClick={() => openEdit()} className="btn-primary"><Plus className="h-4 w-4" /> {t('common.addFloor')}</button>
      </div>
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>{[t('common.name'), t('common.building'), t('common.project'), t('common.locationCode'), t('common.status'), ''].map(h => <th key={h} className="px-4 py-3 text-start text-xs font-semibold text-slate-500 uppercase">{h}</th>)}</tr>
          </thead>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? `${t('common.edit')} ${t('nav.floors')}` : t('common.addFloor')}>
        <form onSubmit={e => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
          <div><label className="block text-sm font-medium text-slate-700 mb-1">{t('common.floorName')}</label><input className="input" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required /></div>
          ...
          <div><label className="block text-sm font-medium text-slate-700 mb-1">{t('common.building')}</label>
